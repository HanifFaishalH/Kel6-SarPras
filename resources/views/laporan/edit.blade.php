        ...
        <div class="modal-body">
            <form id="formEditLaporan" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="gedung_id" id="gedung_id" class="form-control" required>
                            ...
                            @foreach($gedung as $g)
                                <option value="{{ $g->gedung_id }}" {{ $laporan->gedung_id == $g->gedung_id ? 'selected' : '' }}>{{ $g->gedung_nama }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="lantai_id" id="lantai_id" class="form-control" required>
                            ...
                            @foreach($lantai as $l)
                                <option value="{{ $l->lantai_id }}" {{ $laporan->lantai_id == $l->lantai_id ? 'selected' : '' }}>{{ $l->lantai_nama }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="ruang_id" id="ruang_id" class="form-control" required>
                            ...
                            @foreach($ruang as $r)
                                <option value="{{ $r->ruang_id }}" {{ $laporan->ruang_id == $r->ruang_id ? 'selected' : '' }}>{{ $r->ruang_nama }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="sarana_id" id="sarana_id" class="form-control" required>
                            ...
                            @foreach($sarana as $s)
                                <option value="{{ $s->sarana_id }}" {{ $laporan->sarana_id == $s->sarana_id ? 'selected' : '' }}>{{ $s->barang->barang_nama ?? 'Sarana #' . $s->sarana_id }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" name="laporan_judul" class="form-control" value="{{ $laporan->laporan_judul }}" required>
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-sm-3 col-form-label">Foto Laporan</label>
                    <div class="col-sm-9">
                        <input type="file" name="laporan_foto" class="form-control-file">
                        @if($laporan->laporan_foto)
                            <p>Aktual: <a href="{{ asset($laporan->laporan_foto) }}" target="_blank">Lihat Foto</a></p>
                        @endif
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="tingkat_kerusakan" class="form-control" required>
                            <option value="rendah" {{ $laporan->tingkat_kerusakan == 'rendah' ? 'selected' : '' }}>Rendah</option>
                            <option value="sedang" {{ $laporan->tingkat_kerusakan == 'sedang' ? 'selected' : '' }}>Sedang</option>
                            <option value="tinggi" {{ $laporan->tingkat_kerusakan == 'tinggi' ? 'selected' : '' }}>Tinggi</option>
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="tingkat_urgensi" class="form-control" required>
                            ...
                            <option value="kritis" {{ $laporan->tingkat_urgensi == 'kritis' ? 'selected' : '' }}>Kritis</option>
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <select name="frekuensi_penggunaan" class="form-control" required>
                            <option value="harian" {{ $laporan->frekuensi_penggunaan == 'harian' ? 'selected' : '' }}>Harian</option>
                            <option value="mingguan" {{ $laporan->frekuensi_penggunaan == 'mingguan' ? 'selected' : '' }}>Mingguan</option>
                            <option value="bulanan" {{ $laporan->frekuensi_penggunaan == 'bulanan' ? 'selected' : '' }}>Bulanan</option>
                            <option value="tahunan" {{ $laporan->frekuensi_penggunaan == 'tahunan' ? 'selected' : '' }}>Tahunan</option>
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <input type="date" name="tanggal_operasional" class="form-control" value="{{ $laporan->tanggal_operasional }}" required>
                    </div>
                </div>
            </form>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-primary" onclick="submitEditLaporan()">Simpan</button>
        </div>
    </div>
</div>
...
<script>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $.each(data.ruang, function(key, value) {
                            $('#ruang_id').append('<option value="' + value.ruang_id + '">' + value.ruang_nama + '</option>');
                        });
    ...
    function submitEditLaporan() {
        var formData = new FormData($('#formEditLaporan')[0]);
        $.ajax({
            url: '{{ url("/laporan") }}/' + $('#formEditLaporan input[name="laporan_id"]').val() + '/update_ajax',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
